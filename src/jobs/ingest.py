...
from src.config.settings import RAW_DIR, CURATED_DIR, DEFAULT_SHUFFLE_PARTITIONS, AUDIT_DIR
...
def ingest(dt: str, force: bool = False) -> dict:
    logger = get_logger("ingest", dt=dt, base_dir=DATA_DIR)
    ...
    # Idempotency: skip if curated already exists
    if (not force) and _parquet_exists(out_path):
        ...
        audit_path = write_audit(
            ...
            payload={
                "dt": dt,
                "status": "skipped",
                "reason": "curated_exists",
                "raw_path": raw_path,
                "curated_path": out_path,
            },
        )
        logger.warning(f"[ingest] audit: {audit_path}")
    ...
    try:
        ...
        logger.info(f"[ingest] dt={dt} raw={raw_cnt:,} valid={valid_cnt:,} dedup={dedup_cnt:,} dup_rate={dup_rate:.4f} elapsed={elapsed:.2f}s")
        logger.info(f"[ingest] wrote: {out_path}")
        ...
        logger.info(f"[ingest] audit: {audit_path}")
        ...
        logger.exception(f"[ingest] FAILED dt={dt}. audit: {audit_path}")
        raise
    finally:
        spark.stop()
